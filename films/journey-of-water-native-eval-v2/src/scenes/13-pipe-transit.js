...
FILM.scene({
  id:'pipe-transit',
  draw(ctx,t,info){
    const L=info.lib,P=L.pal,p=L.clamp(t/info.dur),q=L.clamp(L.onTwos(t)/info.dur),seed=L.hash('pipe-transit');
    ...
    L.text(ctx,'STUB 13',540,330,{size:60,weight:600,align:'center',color:P.annMagenta});
    L.text(ctx,info.shot.title||'pipe-transit',540,1420,{size:44,align:'center',color:P.ink});
