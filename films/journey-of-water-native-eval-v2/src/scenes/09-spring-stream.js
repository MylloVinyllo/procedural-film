...
FILM.scene({
  id:'spring-stream',
  draw(ctx,t,info){
    const L=info.lib,P=L.pal,p=L.clamp(t/info.dur),q=L.clamp(L.onTwos(t)/info.dur),seed=L.hash('spring-stream');
    ...
    L.text(ctx,info.shot.title||'spring-stream',540,1420,{size:44,align:'center',color:P.ink});
