...
FILM.scene({
  id:'cloud-loop',
  draw(ctx,t,info){
    const L=info.lib,P=L.pal,p=L.clamp(t/info.dur),q=L.clamp(L.onTwos(t)/info.dur),seed=L.hash('cloud-loop');
    ...
    L.text(ctx,'STUB 20',540,330,{size:60,weight:600,align:'center',color:P.annMagenta});
    L.text(ctx,info.shot.title||'cloud-loop',540,1420,{size:44,align:'center',color:P.ink});
