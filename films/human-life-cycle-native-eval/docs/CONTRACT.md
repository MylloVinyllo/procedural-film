...
A film of the human life cycle, drawn entirely by JavaScript on a canvas, with music and sound effects synthesised in JavaScript.
It ships as one self-contained HTML file and as a rendered MP4 for YouTube Shorts.
The look and editing follow `docs/art-bible.md`.\n\nNative evaluation brief: the film must include the social human in comparison with the individual human, cyclicity, and symbolism. Default duration: approximately 30 seconds. The agent invents all other creative decisions within the Procedural Film house style.
