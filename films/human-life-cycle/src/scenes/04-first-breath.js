// STUB
// Placeholder for shot 04 'first-breath' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'first-breath',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('first-breath');
    L.paper(ctx);
    ...
    L.text(ctx, 'STUB 04', 540, 330, { size: 60, weight: 600, align: 'center', color: P.annMagenta });
    L.text(ctx, info.shot.title || 'first-breath', 540, 1420, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'first-breath', 540, 1480, { size: 30, align: 'center', color: P.inkSoft });
