// STUB
// Placeholder for shot 09 'one-among-many' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'one-among-many',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('one-among-many');
    L.paper(ctx);
    ...
    L.text(ctx, 'STUB 09', 540, 330, { size: 60, weight: 600, align: 'center', color: P.annMagenta });
    L.text(ctx, info.shot.title || 'one-among-many', 540, 1420, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'one-among-many', 540, 1480, { size: 30, align: 'center', color: P.inkSoft });
