// STUB
// Placeholder for shot 03 'forming-human' (illustrated). The scene agent replaces this whole file.
FILM.scene({
  id: 'forming-human',
  draw(ctx, t, info) {
    ...
    const seed = L.hash('forming-human');
    L.paper(ctx);
    ...
    L.text(ctx, 'STUB 03', 540, 330, { size: 60, weight: 600, align: 'center', color: P.annMagenta });
    L.text(ctx, info.shot.title || 'forming-human', 540, 1420, { size: 44, align: 'center', color: P.ink });
    L.text(ctx, 'forming-human', 540, 1480, { size: 30, align: 'center', color: P.inkSoft });
